...
func NewCmdPurge() *xli.Command {
	return &xli.Command{
		...
		Handler: xli.OnRun(func(ctx context.Context, cmd *xli.Command, next xli.Next) error {
			...
			// 1. archive home (best effort).
			if home != "" {
				if archive, err := archiveHome(ctx, runner, user, home); err != nil {
					fmt.Fprintf(errWriter(cmd), "warning: home archive failed: %v\n", err)
				} else if archive != "" {
			...
			if err := samba.New(runner).Delete(ctx, user); err != nil {
				fmt.Fprintf(errWriter(cmd), "warning: smbpasswd -x failed (no SMB account?): %v\n", err)
			}
			...
			if _, err := runner.Run(ctx, "", "groupdel", user); err != nil {
				fmt.Fprintf(errWriter(cmd), "note: groupdel %s: %v (UPG may have been removed by userdel)\n", user, err)
			}

			cmd.Printf("purged user %q (uid %d)\n", user, uid)

			// 5. reserve the uid so it is never reused.
			doReserve, _ := flg.Get[bool](cmd, "reserve")
			if doReserve {
				path := "roster.yaml"
				...
				if err := reserveInRoster(path, user, uid); err != nil {
					fmt.Fprintf(errWriter(cmd), "warning: could not record reserved tombstone in %s: %v\n", path, err)
...
func errWriter(cmd *xli.Command) *os.File {
	if cmd.ErrWriter != nil {
		if f, ok := cmd.ErrWriter.(*os.File); ok {
			return f
		}
	}
	return os.Stderr
