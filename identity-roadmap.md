...
파일서버가 usersync를 **컨테이너 안에서 부팅마다 적용**하는 형태로 배포된다면(darak의 ADR-9), 위 절차 중 두 단계가 하나로 합쳐진다.

로컬 계정이 매 부팅 재생성되는 구조이므로, 인계는 사용자별 `detach`가 아니라 **"더 이상 만들지 않기"** 로 표현된다:

```
usersync.yaml 에 mode: audit  →  재시작  →  로컬 계정이 생성되지 않음  →  winbind 가 그 이름을 답한다
```

Step 3(한 명씩 `detach`)과 Step 4(`mode: audit` 전환)가 재시작 한 번이 된다.

**대신 한 명씩 검증하며 넘어갈 수 없다** — 전원이 동시에 넘어간다. 단계적으로 가고 싶다면 넘긴 사람을 roster에서 빼는 방식으로 나누거나, 호스트 배포처럼 `detach` 를 쓴다. 처음 조인하는 날에는 후자를 권한다: `detach` 가 직후에 이름을 재조회해 **다른 uid 로 해석되면 에러로 멈추는** 검증을 하는데, 재시작 방식에는 그 검증이 없다.

**Step 5의 권장안이 이 형태에서는 성립하지 않는다.** 로컬 `/etc/group` 의 UPG 항목도 매 부팅 사라지므로, "로컬 UPG 를 남겨 이름 해석을 복구한다"를 쓰려면 `mode: audit` 이후에도 그룹만은 계속 적용해야 한다. 그럴 바에는 AD 에 사용자당 그룹 객체를 만드는 쪽이 정직하다.

**부팅 스크립트가 `mode` 를 봐야 한다.** `mode: audit` 에서는 `usersync apply` 가 거부하므로, 그대로 두면 전환한 날 컨테이너가 기동하지 못한다. `usersync config` 로 mode 를 읽어 `apply` 대신 `audit` 을 돌리도록 분기할 것.
