...
func TestFindings(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			rep := Run(declared(), resolved(tt.users, tt.groups), classifier())
			if rep.OK() {
...
func TestIgnoresOutOfBandCollision(t *testing.T) {
	rep := Run(&roster.Roster{}, resolved(
...
func TestDeterministicOrder(t *testing.T) {
	...
	first := render(Run(declared(), resolved(users, groups), classifier()))
	for i := 0; i < 20; i++ {
		if got := render(Run(declared(), resolved(users, groups), classifier())); got != first {
